...
class ClientAgent:
    ...
    def create_agent(self) -> Agent:
        ...
        return Agent(
            ...
            description="A crypto trading assistant that buys signals from the DeepBlue signal agent.",
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        ...
        return f"""
You are a crypto trading assistant. Your job is to help users get live 5-minute directional trading signals for BTC, ETH, SOL, and XRP.

**Standard Operating Procedure (SOP):**

1.  **Discover**: Always start by using `list_remote_agents` to see which agents are available.
2.  **Ask for coin**: If the user hasn't specified a coin, ask them which one they want a signal for (BTC, ETH, SOL, or XRP).
3.  **Delegate**: Send the request to the signal agent using `send_message`.
4.  **Confirm Payment**: If the signal agent requires a payment, the system will return a confirmation message. Present the cost clearly to the user and ask if they want to proceed.
5.  **Sign and Send**: If the user confirms (e.g., "yes", "proceed", "pay"), call `send_message` again targeting the *same agent* with the exact message: "sign_and_send_payment". The system will handle signing and sending the payment.
6.  **Report Signal**: Clearly present the final signal result — coin, direction (UP or DOWN), confidence score, and any additional context.
...
"""
    ...
    async def send_message(
        ...
        if response_task.status.state == TaskState.input_required:
            ...
            currency_name = payment_option.extra.get("name", "USDC")
            signal_meta = payment_option.extra.get("signal", {})
            coin = signal_meta.get("coin", "the requested coin")

            return (
                f"The signal agent is requesting payment for a {coin} 5-minute signal. "
                f"Cost: {currency_amount} {currency_name} atomic units (~$0.002 USDC). "
            ...
            # Fallback for tasks with no text artifacts
            if (
                self.x402.get_payment_status(response_task)
                == PaymentStatus.PAYMENT_COMPLETED
            ):
                return "Payment successful! Your signal has been delivered."
