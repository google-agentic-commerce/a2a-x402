...
class SimpleHostAgent:
    ...
    def __init__(self, batch_mode=False):
        ...
        print(f"[Host] Initialized with batch_mode={batch_mode}")
    ...
    async def ask_merchant(self, merchant_name: str, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if payment_status == PaymentStatus.PAYMENT_REQUIRED:
                ...
                # Extract message from completed task
    ...
    async def _handle_payment_required(self, task, client, merchant_name: str, original_question: str = ""):
        ...
        try:
            ...
            if self.batch_mode:
                ...
                payment_payload = process_payment(first_requirement, self.sui_client)
                if payment_payload:
                    return await self._submit_payment(task, client, merchant_name, payment_payload)
                else:
    ...
    async def process_batch_payments(self) -> Dict[str, Any]:
        ...
        for i, (task_id, data) in enumerate(self.pending_payments.items()):
            ...
            try:
                result = await self._submit_payment(task, client, merchant_name, payment_payload)
                results.append({
                    "task_id": task_id,
                    "merchant": merchant_name,
                    "status": result.get("status", "unknown"),
                    "message": result.get("message", ""),
                    "transaction_hash": self._extract_tx_hash(result.get("message", ""))
                })
            except Exception as e:
                results.append({
                    "task_id": task_id,
    ...
    async def _submit_payment(self, task, client, merchant_name: str, payment_payload):
        ...
        if hasattr(payment_response, 'root') and hasattr(payment_response.root, 'result'):
            ...
            message = self._extract_task_message(final_task)
            
            return {
                "status": "success" if final_task.status.state == "completed" else "partial",
                "message": message,
...
host_agent = SimpleHostAgent(batch_mode=batch_mode)